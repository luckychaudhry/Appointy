  ...
  return (
    <div className='flex items-center justify-between text-sm pt-2 pb-0 border-b border-b-gray-400'>
      ...
      <ul className='hidden md:flex items-start gap-5 font-medium'>
        ...
        <li>
          <NavLink to='/health-analyzer' className={({ isActive }) => isActive ? 'border-b-2 border-primary' : ''}>HEALTH ANALYZER</NavLink>
        </li>
        {/* <li>
          <NavLink to="/ai-checker">AI Checker</NavLink>
        </li> */}
      </ul>
      ...
      <div className='flex items-center gap-4'>
        ...
        <div className={`md:hidden ${showMenu ? 'fixed w-full' : 'h-0 w-0'} right-0 top-0 bottom-0 z-20 overflow-hidden bg-white transition-all`}>
          ...
          <ul className='flex flex-col items-center gap-2 mt-5 px-5 text-lg font-medium'>
            ...
            <NavLink onClick={() => setShowMenu(false)} to='/health-analyzer' ><p className='px-4 py-2 rounded full inline-block'>HEALTH ANALYZER</p></NavLink>
          </ul>
        </div>
      </div>
    </div>
